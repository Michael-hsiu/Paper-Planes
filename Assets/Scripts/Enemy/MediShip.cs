...
public class MediShip : Ship
{
    ...
    protected void HealAlly(GameObject go)
    {
        ...
        if (Physics.Raycast(transform.position, rayDirection, out hit, followDistance))
        {   // Can only heal when nearby
            ...
            if (currTargets.Count > 0)
            {
                ...
                foreach (GameObject go in currTargets)
                {
                    ...
                    if (s != null)
                    {
                        ...
                        else if (s.health < mostDamaged.health)
                        {       // Need to adjust to percentages
                            mostDamaged = s;
                        }
                    }
                }
