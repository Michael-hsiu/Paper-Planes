...
public class VirtualJoystick : MonoBehaviour,IDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	public virtual void OnDrag(PointerEventData ped) {
																	...
																	out pos)) 
		{
			...
			float x = 0f;
			if (bgImg.rectTransform.pivot.x == 1) {		// If on right
				x = pos.x * 2 + 1;
			} else {									// If on left
				x = pos.x * 2 - 1;
			}
			...
			if (bgImg.rectTransform.pivot.y == 1) {		// If on right
				y = pos.y * 2 + 1;
			} else {									// If on left
				y = pos.y * 2 - 1;
			}
			...
			joystickImg.rectTransform.anchoredPosition = new Vector3 (inputDirection.x * (bgImg.rectTransform.sizeDelta.x / 3), inputDirection.z * (bgImg.rectTransform.sizeDelta.y / 3));		// Move front knob by a third of bgimg size
